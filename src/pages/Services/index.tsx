    ...
    <div className="pt-32 bg-white dark:bg-dark transition-colors">
      ...
      <section className="py-24 bg-slate-50 dark:bg-dark relative overflow-hidden grid-bg border-b border-slate-200 dark:border-white/5 mb-32 transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <h1 className="text-6xl md:text-9xl font-black text-slate-900 dark:text-white tracking-tighter mb-8 leading-tight uppercase">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="space-y-60">
          <div className="space-y-60">
            <ModuleSection {...SERVICES[0]} image="/hosted.png" t={t} />
            <ModuleSection
              {...SERVICES[1]}
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-24 flex flex-col lg:flex-row justify-between items-end gap-8">
            <h3 className="text-5xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">
              {t("services:page.engagementTitle")}
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-1 bg-slate-200 dark:bg-white/5 border border-slate-200 dark:border-white/5">
            <div className="md:col-span-4 bg-white dark:bg-dark p-12 transition-colors">
              <EngagementCard
                name={t("services:page.engagementCards.audit.name")}
                price={t("services:page.engagementCards.audit.price")}
                desc={t("services:page.engagementCards.audit.description")}
                t={t}
              />
            </div>
            <div className="md:col-span-4 bg-primary p-12 relative overflow-hidden shadow-2xl shadow-primary/20">
              <div className="absolute top-0 right-0 p-4 text-[10px] font-black text-white/40 tracking-widest uppercase rotate-90 origin-top-right translate-y-8">
                RECOMMENDED
              </div>
              <EngagementCard
                name={t("services:page.engagementCards.project.name")}
                price={t("services:page.engagementCards.project.price")}
                desc={t("services:page.engagementCards.project.description")}
                featured
                t={t}
              />
            </div>
            <div className="md:col-span-4 bg-white dark:bg-dark p-12 transition-colors">
              <EngagementCard
                name={t("services:page.engagementCards.managed.name")}
                price={t("services:page.engagementCards.managed.price")}
                desc={t("services:page.engagementCards.managed.description")}
                t={t}
              />
            </div>
          </div>
    ...
    <div className={`lg:col-span-6 ${isReversed ? "lg:order-2" : ""}`}>
      <div className="flex items-center space-x-4 mb-8">
        ...
        <div>
          <span className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">
            {t(`services:services.${id}.tag`)}
          </span>
          <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tighter uppercase transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-12 mb-10">
        {features.map((f, i) => (
          <div
            key={i}
            className="flex items-center space-x-3 text-[11px] text-slate-600 dark:text-gray-300 font-black uppercase tracking-widest"
          >
      ...
      <Link
        to={`/services/${slug}`}
        className="inline-block py-4 px-10 border border-primary/30 text-primary text-[10px] font-black tracking-[0.4em] uppercase hover:bg-primary hover:text-white transition-all duration-300"
      >
        {t("services:page.viewDocs")}
      </Link>
    </div>
    <div
      className={`lg:col-span-6 aspect-square bg-slate-50 dark:bg-dark/40 border border-slate-200 dark:border-white/5 relative flex items-center justify-center overflow-hidden corner-accent transition-colors ${isReversed ? "lg:order-1" : ""} group`}
    >
      <img
        src={image}
      ...
      <div className="absolute top-8 left-8 text-[10px] font-black text-slate-400 dark:text-white/10 tracking-[0.5em] z-10 mix-blend-difference">
        {technicalCode}
      </div>
      <div className="w-3/4 h-3/4 border border-primary/30 p-8 flex flex-col justify-center relative z-10 backdrop-blur-sm bg-slate-900/30">
        ...
        <div className="space-y-4 text-center">
          <div className="text-7xl font-black text-white/20 uppercase select-none">
            {id.charAt(0)}
          </div>
...
const EngagementCard: React.FC<{
  ...
  featured?: boolean;
  t: any;
}> = ({ name, price, desc, featured, t }) => (
  <div className="h-full flex flex-col justify-between">
    <div>
      <h4
        className={`text-2xl font-black mb-2 uppercase tracking-tight ${featured ? "text-white" : "text-slate-900 dark:text-white"}`}
      >
        {name}
      </h4>
      <div
        className={`text-[10px] font-black uppercase tracking-[0.4em] mb-8 ${featured ? "text-white/60" : "text-primary"}`}
      >
        {price}
      </div>
      <p
        className={`text-sm font-light leading-relaxed mb-10 ${featured ? "text-white/80" : "text-slate-500 dark:text-gray-400"}`}
      >
        {desc}
      </p>
    </div>
    <button
      className={`w-full py-5 text-[10px] font-black tracking-[0.4em] transition-all duration-500 uppercase ${featured ? "bg-white text-primary hover:bg-dark hover:text-white" : "border border-primary/40 text-primary hover:bg-primary hover:text-white"}`}
    >
