...
const TimelineItem: React.FC<{ milestone: Milestone; index: number; isLast: boolean }> = ({ milestone, index, isLast }) => {
    const ref = useRef<HTMLDivElement>(null);
    const isInView = useInView(ref, { once: true, margin: "-100px" });
...
const StatsSection: React.FC = () => {
    const stats: StatItem[] = [
        { value: 10, suffix: '+', prefix: '', label: 'yearsExperience', decimals: 0 },
        { value: 150, suffix: '+', prefix: '', label: 'projectsDelivered', decimals: 0 },
        { value: 50, suffix: '+', prefix: '', label: 'happyClients', decimals: 0 },
        { value: 15, suffix: '+', prefix: '', label: 'industriesServed', decimals: 0 },
    ];
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
                    ...
                    >
                        ...
                        <h2 className="text-4xl md:text-6xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-tight mb-8 transition-colors">
                            {t('about:skills.title')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">{t('about:skills.titleHighlight')}</span>
                        </h2>
                        ...
                        <div className="grid grid-cols-3 gap-6">
                            {[
                                ...
                                <div key={i} className="text-center p-4 bg-white dark:bg-dark/50 border border-slate-200 dark:border-white/10">
            ...
            <div className="max-w-5xl mx-auto px-6 relative z-10">
                ...
                >
                    <div className="flex items-center justify-center space-x-4 mb-6">
                        <div className="w-12 h-[1px] bg-secondary"></div>
                        <span className="text-[10px] font-black text-secondary uppercase tracking-[0.6em]">{t('about:timeline.tag')}</span>
                        <div className="w-12 h-[1px] bg-secondary"></div>
                    </div>
...
const ValueBlock: React.FC<{ title: string, desc: string, delay: number }> = ({ title, desc, delay }) => {
    const ref = useRef<HTMLDivElement>(null);
    const isInView = useInView(ref, { once: true, margin: "-50px" });
