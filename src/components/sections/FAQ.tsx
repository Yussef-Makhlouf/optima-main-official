...
const FAQ_KEYS = ['migration', 'compliance', 'scaling', 'integration', 'support'];
                    ...
                    {FAQ_KEYS.map((key, index) => (
                        ...
                        >
                            ...
                            >
                                ...
                                <div className={`w-8 h-8 flex items-center justify-center border border-slate-300 dark:border-white/10 text-primary transition-transform duration-500 ${isRtl ? 'ml-4' : 'mr-4'} ${openIndex === index ? 'rotate-180 bg-primary text-white' : ''}`}>
                            ...
                            >
                                <div className="p-6 md:p-8 pt-0 text-slate-500 dark:text-gray-400 font-light leading-relaxed text-base md:text-lg border-t border-slate-100 dark:border-white/5 mt-2">
                                    <div className={`flex items-start gap-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                                        ...
                                        <p className={isRtl ? 'text-right' : 'text-left'}>{t(`home:faq.items.${key}.answer`)}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
